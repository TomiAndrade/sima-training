...
export default function Results({ usuario, module: mod, result, onRetry, onGoToModules, onHome }) {
  // `result` viene en null hasta que el commit 4 conecte POST /tablet/sesiones
  // y guarde acá el resultado real que calcula el backend — no hay corrección
  // local posible sin `respuestaCorrecta` (ver App.jsx, finishEvaluation).
  const pendienteDeEnvio = result === null

  const approved = result?.approved
  const feedbackMsg = pendienteDeEnvio
    ? null
    : approved
    ...
    <div className="w-full max-w-sm bg-white border border-slate-200 rounded-2xl shadow-2xl p-8">
      {pendienteDeEnvio ? (
        <div className="w-40 h-40 rounded-full border-8 border-slate-300 bg-slate-50 flex flex-col items-center justify-center mb-6 mx-auto text-center px-4">
          <span className="text-slate-500 text-sm font-bold leading-snug">Resultado pendiente de enviar al servidor</span>
        </div>
      ) : (
        <>
          {/* Score circle */}
          <div className={`w-40 h-40 rounded-full border-8 flex flex-col items-center justify-center mb-6 mx-auto ${approved ? 'border-emerald-500 bg-emerald-50' : 'border-red-600 bg-red-50'}`}>
            <span className={`text-5xl font-black ${approved ? 'text-emerald-600' : 'text-red-600'}`}>{result.percentage}%</span>
