...
- ~~**Pendiente para el próximo sprint**: `PATCH /modulos/:id/aprobar`~~ — **resuelto con otro diseño**: el Sprint 3 (versionado) lo reemplazó por `PATCH /modulos/:id/activar`, que publica el borrador directamente (sin un paso de aprobación separado). El AuditLog **ya está implementado, parcialmente** (Story 9, ver "Decisiones de diseño" más abajo): cubre `Vinculacion` y `VinculacionPuestoCentro`; extenderlo al resto queda en `docs/pendientes.md`.
...
- **El registro de aprobaciones ya está implementado** (Sprint 8, más abajo) y cerró el hueco que este bullet describía: `modulosAprobados()` consulta las `Sesion` aprobadas —leyendo por el cliente transaccional que le pasa `recalcularEnTx`— y `Asignacion.moduloVersionId` lo completa `SesionesService.registrar()` al aprobar. El paso de revocación **no** cambió: sigue comparando contra `requeridos` sin restarle los aprobados, así que un módulo aprobado que una regla sigue pidiendo no se revoca, sólo no se re-crea. Desde Story 8, además, "aprobado" ya no es para siempre: `modulosAprobados()` descarta las aprobaciones cuyo vencimiento pasó (ver "Decisiones de diseño (Story 8 — Vigencia de aprobaciones)" más abajo) — el paso de creación se beneficia solo, sin ningún cambio de código ahí.
...
- **El PATCH de `pares` reemplaza el set completo, no mergea**: borra los que había y crea los de la lista, en una transacción y borrando antes de crear (por el índice único parcial de `principal`). Omitir `pares` los deja intactos. La fila de `VinculacionPuestoCentro` sigue borrándose físico —eso no cambió—, pero desde Story 9 ese borrado queda auditado en `AuditLog` dentro de la misma transacción, así que "qué pares tenía antes" ahora tiene dónde consultarse aunque la fila ya no exista.
...
- **Un reintento es una fila más: NO hay unicidad sobre (usuario, moduloVersion).** Es la diferencia con `Asignacion`, donde el índice parcial deja a lo sumo una vigente. "¿Aprobó?" es `EXISTS(aprobada)` sobre **cualquier** versión del módulo (la obligación es "este módulo") y "¿cuál fue el último intento?" se ordena por `finalizadaEn`: las dos derivadas, sin ningún flag `vigente` que mantener consistente. Un intento fallido **no des-aprueba** — volver a rendir y desaprobar no invalida la aprobación anterior. Lo que sí la caduca es el tiempo: desde Story 8, `modulosAprobados()` deja de contar una aprobación cuyo vencimiento ya pasó (ver "Decisiones de diseño (Story 8 — Vigencia de aprobaciones)", más abajo).
...
## Decisiones de diseño (Story 8 — Vigencia de aprobaciones)

Hace que `Modulo.vigenciaMeses` gobierne de verdad. Cuatro pasos: `src/asignaciones/vigencia.ts` (funciones puras: `sumarMeses`, `calcularVencimiento`) → `AsignacionesService.aprobacionesPorModulo()`/`modulosAprobados()` filtran por vencimiento → `GET /asignaciones?usuarioId=` expone `vencimiento` por fila → esta sección.

- **El vencimiento es DERIVADO, no un evento**: sin job programado ni columna `vencidaAt`. Se calcula al leer, contra `Sesion.createdAt` de la última aprobación y `Modulo.vigenciaMeses` vivo. Alcanza porque **aprobar no revoca la `Asignacion`**: cuando una aprobación caduca no hay ninguna fila que crear, sólo cambia la respuesta a "¿está cumplido?" la próxima vez que se pregunta.
- **La fecha es `Sesion.createdAt` (servidor, no `finalizadaEn` del dispositivo) de la aprobación MÁS RECIENTE del módulo.** `aprobacionesPorModulo()` agrupa en JS y se queda con el `createdAt` más alto por `moduloId`: aprobar de nuevo reinicia el reloj de la vigencia.
- **`vigenciaMeses` se lee VIVO de `Modulo`, no se congela en `Sesion`** — a diferencia de `umbralAprobacion` (Sprint 8, más arriba), que sí se congela. El umbral decide un veredicto ya emitido; la vigencia decide una obligación futura, y si HSE la acorta por un cambio de norma quiere que le venza antes a todo el mundo, no sólo a quien apruebe de ahora en más.
- **`vigenciaMeses` null (o `0`) = no vence nunca** — el default de todos los módulos cargados hoy, así que esta story no cambia nada hasta que se carguen vigencias reales.
- **`recalcular()` no tocó el paso de revocación**; el de creación se beneficia solo (una aprobación vencida deja de tapar la creación, sin código nuevo ahí). Deja de ser determinista respecto de la fecha —la misma base, en dos días distintos, puede dar otro resultado— pero sigue siendo idempotente en el sentido que importa: correrlo dos veces el mismo día no duplica ni revoca de más.
...
## Decisiones de diseño (Story 9 — Auditoría)

- **El diff se guarda, no un snapshot**: `{ campo: { antes, despues } }`, sólo lo que cambió — evita tener que comparar dos filas a mano para saber qué pasó.
- **`actor` es el mismo string genérico que ya usa `createdBy`** (`'backoffice'`/`'import'`/`'tablet'`), porque no existen cuentas de sistema todavía. Lo irrecuperable es el diff, no el actor: el actor se puede completar después, el diff no. La columna es `String` libre, acepta un id más adelante sin migrar.
- **Alcance de esta primera pasada: sólo `Vinculacion` y `VinculacionPuestoCentro`**, lo único que hoy perdía información al editarse.
- **Hook explícito desde el service, dentro de la transacción del cambio** — no un middleware de Prisma ni un interceptor de Nest: no sabrían distinguir un cambio real de uno cosmético ni quién lo hizo, y `tx` es obligatorio en `registrar()` para que un rollback se lleve el log puesto.
