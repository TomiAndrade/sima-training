...
| `POST` | `/asignaciones/recalcular/:usuarioId` | JWT | Deriva las AUTOMATICA a partir de los pares (puesto, centro) activos y las reglas vigentes: crea las que faltan y revoca las que ya no corresponden. Nunca toca las MANUAL. Devuelve `{ creadas, revocadas }`. Editar los pares de una persona **ya lo dispara solo** (`UsuariosService` lo invoca dentro de la misma transacción); este endpoint queda para recalcular a demanda, sobre todo después de tocar una **regla**, que no recalcula a nadie |
...
- ⚠️ **Hueco conocido: tocar una REGLA no recalcula a nadie.** `ReglasAsignacionService` no inyecta el motor, así que crear/activar/desactivar una `ReglaAsignacion` deja desactualizadas a las personas alcanzadas hasta que alguien toque sus pares o llame a `POST /asignaciones/recalcular/:usuarioId` (el botón "Recalcular" del backoffice). Recalcular en masa al tocar una regla no se implementó porque el fan-out puede ser grande y no hay job en background.
