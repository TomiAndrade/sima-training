...
│                    SIN controller todavía — los endpoints de la tablet son la
│                    Story 5 del sprint 07-08
...
Es la Story 4 del sprint 07-08: la entidad que faltaba desde el Sprint 2. El backend sabía **qué le corresponde rendir** a cada persona (`Asignacion`) pero no registraba que hubiera rendido, y eso bloqueaba las aprobaciones, el informe de usuario, los vencimientos y las estadísticas por base. Cerró tres huecos declarados en el propio código: `AsignacionesService.modulosAprobados()`, `Asignacion.moduloVersionId` y el registro de aprobaciones de [`../docs/pendientes.md`](../docs/pendientes.md). **`SesionesModule` todavía no tiene controller**: los endpoints que consume la tablet son la Story 5.
...
- **Sin `claveIdempotencia` todavía.** La idempotencia del POST es de la Story 5 y su consumidor no existe hasta que exista el endpoint — sería otra columna nullable sin uso, el patrón que el proyecto ya rechazó con `cantidadPreguntas`. Cuando vaya, alcanza un `@unique` plano: en Postgres varios NULL no colisionan, así que no hace falta un sexto índice parcial (el `@@unique([sesionId, preguntaId])` de `Respuesta` tampoco lo es, por el mismo motivo: ninguna de sus dos columnas es nullable).
