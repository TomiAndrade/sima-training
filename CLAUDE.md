...
- **Usuarios** — **100% backend, ya migrado al modelo de vinculación**: lee y escribe la forma anidada (`usuario.vinculacion.rol` / `.organizacion` / `.pares` / `.parPrincipal`), no los campos planos que el backend dejó de exponer. Tres tabs (**Todos** / **Alumnos** / **Operadores**), misma tabla/ABM, filtradas por el **rol de la vinculación** — no por tipo de organización: "Alumnos" es `rol = ALUMNO` y "Operadores" es `ADMINISTRADOR | COORDINADOR`. Lo que la tab predefine al crear es el **rol** (`defaultRolParaTab`: ALUMNO en la tab de alumnos, COORDINADOR en las otras); la **organización** se predefine siempre a la primera de la lista, y el select del form las muestra **todas** sin filtrar (más "— Sin organización —"). Incluye la sección de pares puesto/centro (ver "ABM de pares" más arriba) e "Importar Excel".
...
- **El seed borra en orden de dependencia**: `VinculacionPuestoCentro` → `Vinculacion` → `Usuario`/`Organizacion`. Todas las FK son `ON DELETE RESTRICT`; el orden viejo (`usuario.deleteMany()` primero) sobrevivía solo porque la base no tenía vinculaciones — con una sola vinculación sembrada revienta con `vinculaciones_usuario_id_fkey`.
