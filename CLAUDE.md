...
| `ReglaAsignacion` | `id (uuid), puestoId, centroCostoId, moduloId` (`@@unique` los tres), `activo` + trazabilidad. El **par exacto** (puesto, centro de costo) obliga a rendir un módulo. Apunta a `Modulo` (el contenedor), no a una `ModuloVersion` — la obligación es "este módulo", la versión concreta se resuelve al rendir. Baja lógica: reactivar un triple dado de baja actualiza la fila existente en vez de duplicar |
...
Pendiente para el próximo sprint: AuditLog completo (ISO 9001), migrar los **frontends** a la forma nueva de `GET /usuarios` (el rol dejó de ser plano y la clasificación ya no existe — el backoffice todavía consume la forma vieja), compartir un borrador como versión beta para testear antes de publicar (diseño esbozado, sin implementar — ver Sprint 3 más abajo), auto-invocar `recalcular()` desde el ABM de usuarios (hoy es un endpoint aparte, ver más abajo) y el registro de aprobaciones (bloqueado hasta conectar la app tablet). Pantallas de backoffice para `Asignacion`/`ReglaAsignacion` — el modelo ya existe, la UI todavía no. La detección de duplicados de preguntas y el versionado/aprobación de módulos (`PATCH /modulos/:id/activar`) ya están resueltos (ver más arriba); el unassign duro de pregunta↔módulo también se resolvió, ver Sprint 3. Detalle completo de pendientes en [`docs/pendientes.md`](docs/pendientes.md).
...
- **No se dispara solo.** Hoy es un endpoint aparte (`POST /asignaciones/recalcular/:usuarioId`); editar los pares de una persona desde `/usuarios` no lo invoca todavía — se dejó afuera a propósito para no meter un cambio transversal junto con el modelo nuevo (ver pendientes).
