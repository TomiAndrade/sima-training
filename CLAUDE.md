...
| `Pregunta` | `id (uuid), texto, tipo` (VERDADERO_FALSO/OPCION_MULTIPLE/OPCIONES_IMAGEN/TEXTO_LIBRE), `opciones?` (jsonb), `respuestaCorrecta?, imagen?, puntajeMax?, activa` + trazabilidad. Única: se comparte entre módulos, nunca se duplica. `imagen` es la **imagen del enunciado**: guarda una **clave opaca** de storage (`preguntas/<uuid>.png`, se sirve bajo `/uploads`) o una ruta legacy relativa a `public/` si vino del import de Excel. Se setea solo al crear y es **inmutable** (ver Sprint 4) |
...
- **Pendiente relacionado**: las opciones de `OPCIONES_IMAGEN` siguen siendo rutas tipeadas a mano en el form — mismo problema, otra story. El `StorageService` ya queda listo para cuando se haga.
