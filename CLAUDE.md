...
- **Sacar preguntas del borrador**: `PreguntasAsignadasPanel` (compartido entre Preguntas y Capacitaciones) acepta un `onToggle` opcional — cuando se pasa, cada fila suma Activar/Desactivar y las inactivas se muestran atenuadas con badge "Inactiva". La vista de borrador de Capacitaciones lo usa (llama `setPreguntaActiva`, que gracias al punto anterior cae siempre en el borrador correcto); las vistas read-only (ver vigente, detalle del historial) no lo pasan y quedan de solo lectura.
