...
| Entidad | Archivo | Campos |
|---|---|---|
| `TrainingModule` | `sima-check/data/training-modules.js` | `id, backendId, name, active, questions[]` (`questions[]` vacío, ver arriba). **La pantalla Capacitaciones (`TrainingModules.jsx`) ya no usa este mock**: lista/crea/edita módulos y su versionado 100% contra `/modulos`. El archivo sigue siendo consumido, solo para metadata liviana (`name`/`active`), por `Dashboard.jsx`, `Overview.jsx` y `TrainingAssignments.jsx` — pendiente de migrar; mientras tanto esos conteos/selects pueden divergir del estado real del backend |
...
- **Limitación conocida — `Questions.jsx` (pantalla Preguntas) no es borrador-aware**: su vista por-módulo (`QuestionsTableModulo`/`useBancoModulo` sin `versionId`) sigue resolviendo contra la versión **vigente** (ACTIVO-o-última), igual que antes de Sprint 3. Antes esto era inofensivo porque nunca coexistían un ACTIVO y un BORRADOR distintos; ahora que Capacitaciones puede dejar un borrador en curso mientras el módulo sigue publicado, entrar a esa pantalla y togglear una pregunta ahí **afecta al ACTIVO publicado, no al borrador que se está editando** — un gotcha real, no solo teórico. Mientras no se resuelva, editar contenido de un módulo con versionado en curso conviene hacerlo únicamente desde Capacitaciones ("Continuar borrador"), no desde Preguntas.
