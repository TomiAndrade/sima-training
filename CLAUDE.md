...
Pendiente para el próximo sprint: AuditLog completo (ISO 9001), migrar los **frontends** a la forma nueva de `GET /usuarios` (el rol dejó de ser plano y la clasificación ya no existe — el backoffice todavía consume la forma vieja), el ABM de pares (puesto, centro) en el form de Usuario, y compartir un borrador como versión beta para testear antes de publicar (diseño esbozado, sin implementar — ver Sprint 3 más abajo). La detección de duplicados de preguntas y el versionado/aprobación de módulos (`PATCH /modulos/:id/activar`) ya están resueltos (ver más arriba); el unassign duro de pregunta↔módulo también se resolvió, ver Sprint 3.
