...
**Asignaciones automáticas por par (puesto, centro) → módulo:** `ReglaAsignacion` fija qué módulo es obligatorio, por par exacto o por centro (ver tabla de entidades arriba). `AsignacionesService.recalcular(usuarioId)` es el motor: toma los pares ACTIVOS de la vinculación, arma la unión de módulos requeridos por las reglas vigentes que matchean algún par **más** las reglas de centro de los centros donde tiene algún par activo, y la compara contra las `Asignacion` vigentes actuales — crea las `AUTOMATICA` que faltan y revoca las `AUTOMATICA` que ya no corresponden a ningún par activo. Es síncrono e idempotente, y **nunca toca las `MANUAL`** (las que carga un admin a mano). **Se auto-invoca** desde `UsuariosService` al crear un usuario con pares, editar sus pares, darlo de baja o revivirlo (`recalcularEnTx`, dentro de la misma transacción) — ya no es solo un endpoint aparte, aunque `POST /asignaciones/recalcular/:usuarioId` se mantiene para recalcular a demanda. El registro de aprobaciones ("ya rindió y aprobó este módulo") todavía no existe como entidad: el hueco está aislado en `AsignacionesService.modulosAprobados()`, que devuelve un Set vacío hasta que la app tablet se conecte y exista el flujo de rendir evaluación.

**Convención: `onDelete` explícito en relaciones Prisma opcionales.** El default de Prisma para una relación `?` es `SetNull` — no siempre es seguro. En `ReglaAsignacion.puesto` (opcional porque `puestoId = null` significa regla de centro) se fuerza `onDelete: Restrict`: con el default, borrar un `Puesto` convertiría en silencio una regla de par en una regla de centro, aplicándola de golpe a todos los puestos de ese centro. Regla general para el schema: toda relación opcional nueva tiene que decidir su `onDelete` a propósito, no heredar el default sin pensarlo.
...
Pendiente para el próximo sprint: AuditLog completo (ISO 9001), compartir un borrador como versión beta para testear antes de publicar (diseño esbozado, sin implementar — ver Sprint 3 más abajo) y el registro de aprobaciones (bloqueado hasta conectar la app tablet). Pantalla de backoffice para `Asignacion` (vigentes/revocadas por persona) — el modelo ya existe, la UI todavía no (la de `ReglaAsignacion` sí está resuelta, ver tabla de pantallas de SIMA CHECK). La detección de duplicados de preguntas y el versionado/aprobación de módulos (`PATCH /modulos/:id/activar`) ya están resueltos (ver más arriba); el unassign duro de pregunta↔módulo también se resolvió, ver Sprint 3. Detalle completo de pendientes en [`docs/pendientes.md`](docs/pendientes.md).
...
- **Usuarios** — dos tabs (**SIMA** / **Clientes**), misma tabla/ABM. Un usuario es "SIMA" si su organización es `tipo = INTERNA`; el resto son de clientes. Al crear desde cada tab, la org se predefine a la correspondiente. El select del form filtra orgs por tab. ⚠️ **Pendiente de migrar al modelo de vinculación**: la pantalla todavía lee `usuario.rol`/`usuario.organizacionId` planos y la clasificación, campos que el backend ya no expone (ahora van en `usuario.vinculacion`).
...
- **Los frontends todavía consumen la forma vieja** (`usuario.rol` plano, `clasificacion`). Migrarlos es trabajo aparte, fuera de este cambio.
...
- **Ya no es solo un endpoint aparte** (`POST /asignaciones/recalcular/:usuarioId` se mantiene para recalcular a demanda). Se agregó el auto-invoke desde `UsuariosService` (crear/editar pares, baja, revivir) en un cambio posterior — ver la nota de "Asignaciones automáticas" más arriba, cerca de la tabla de entidades.
...
- **El backoffice todavía no tiene pantalla para esto.** El modelo (`Asignacion`/`ReglaAsignacion`) y los endpoints ya existen; la pantalla "Asignaciones" de SIMA CHECK sigue siendo el mock viejo de HSE (`training-assignments.js`), sin relación con las tablas reales.
