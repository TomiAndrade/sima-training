...
**El refresco manual tiene estado propio, y no reusa el de la carga inicial.** El botón "↻ Actualizar" existe porque una rendición hecha en la tablet no aparece acá hasta salir y volver a entrar. Lo tentador era reusar el `reintentar()` que ya estaba para el banner de error, pero `loading` y `error` son **early returns que reemplazan la pantalla entera**: eso está bien en la primera carga —todavía no hay nada que perder— y es una regresión al refrescar, porque deja en blanco un informe que se estaba leyendo. Por eso hay un par aparte, `refrescando`/`errorRefresco`.

La misma lógica decide qué pasa cuando el refresco **falla**: el informe viejo se queda en pantalla y el error va a un banner arriba que aclara que son los datos de la última carga. Datos viejos y avisados es mejor que una pantalla en blanco, y bastante mejor que fallar en silencio: quien mira una hoja de vida para decidir si alguien está habilitado necesita saber que lo que ve puede estar desactualizado. El botón no aparece en los early returns —sin informe cargado no hay nada que refrescar, y ese caso ya lo cubre "Reintentar".
